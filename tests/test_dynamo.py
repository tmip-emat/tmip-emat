...
def test_simple_experiments(dynamo_local):
    ...
    out2 = m.read_experiment_measures('lhs5').reset_index()\
        .set_index('experiment_id').drop(columns='run_id')
    ...
    pd.testing.assert_index_equal(design6.index, pd.Index([6,7,8,9,10,11], name='experiment'))
    ...
    db.store_scope(s3)
    assert db.read_scope_names() == ['EMAT Road Test', 'EMAT Road Test v2', 'EMAT Road Test v3']

    design4 = s3.design_experiments(db=db, n_samples=4, design_name="lhs4")

    assert len(db.read_experiment_parameters(s2.domain)) == 15
    assert db.read_scope_names(domain='EMAT Road Test') == ['EMAT Road Test', 'EMAT Road Test v2', 'EMAT Road Test v3']
